...
def train(model, model_unet, dataloader, optimizer, criterion, device, a, b):
    model.train()
    model_unet.eval()  # 设置为评估模式，不训练
    ...
    for forecast, true_rmm, true_var, amp in dataloader:
        ...
        with torch.no_grad():
            features = model_unet(forecast[:,:,:,0:16,:])

        rmm_output = model(features)
        loss = criterion(rmm_output, true_rmm)
...
def test(model, model_unet, dataloader, criterion, device, a, b):
    model.eval()
    model_unet.eval()
...
if __name__ == "__main__":
    ...
    def load_pretrained_weights(model, pretrained_path):
        try:
            ...
            print("不匹配的键:", set(model_dict.keys()) - set(pretrained_dict.keys()))
            
        except Exception as e:
            print(f"加载预训练权重失败: {e}")

    pretrained_path = "/home/sunyuze/08_mjo_corr_new/model_7_new/model_68.pth"
    load_pretrained_weights(model_unet, pretrained_path)
    ...
    for epoch in range(num_epochs):
        train_loss = train(model, model_unet, train_dataloader, optimizer, criterion, device, a, b)
        test_loss = test(model, model_unet, test_dataloader, criterion, device, a, b)

        print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.8f}, Test Loss: {test_loss:.8f}")
        model_save_dir = f'/home/sunyuze/09_mjo_rmm_corr/model_save/model_2'
